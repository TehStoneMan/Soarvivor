...
public class ContainerLimitedPlayer extends LimitedContainer {
    ...
    public ContainerLimitedPlayer(EntityPlayer player,
	...
	for (i = 0; i < 3; ++i)
	    if (i == 0)
		this.addSlotToContainer(new SlotQuiver(inventoryCustom, i,
			8 + i * 18, 84));
	    else
		this.addSlotToContainer(new SlotArrow(inventoryCustom, i,
			8 + i * 18, 84));
	...
	i = 2;
	for (j = 0; j < 9; ++j)
	    this.addSlotToContainer(new SlotLimited(inventoryPlayer, j
		    + (i + 1) * 9, 8 + j * 18, 84 + i * 18));

	// Add ACTION BAR - just copied/pasted from vanilla classes
	for (i = 0; i < 9; ++i)
	    this.addSlotToContainer(new SlotLimited(inventoryPlayer, i,
		    8 + i * 18, 142));
	...
	if (slot != null && slot.getHasStack()) {
		...
		if (!this.mergeItemStack(stackSource, ARMOR_START + type,
